...
import com.evans.technologies.conductor.ui.recover.recoverPass.changepassword
import com.evans.technologies.conductor.ui.recover.recoverPass.correo
import com.evans.technologies.conductor.ui.recover.recoverPass.set_codigo
import com.evans.technologies.conductor.ui.main.view.home.mapaInicio
...
fun passwordvalido(pass:String) =pass.isNotEmpty()&&
        pass.length.compareTo(8)==1
fun userValido(email:String) = email.isNotEmpty()&&
        email.contains('@')
...
fun imagenExiste(path:String)= File("${Constants.APP_SETTINGS_PATH_PHOTOS}$path.jpg").exists()

suspend fun saveToInternalStorage(bitmapImage: Bitmap, name:String):Unit = suspendCancellableCoroutine {
    val path=File(Constants.APP_SETTINGS_PATH_PHOTOS)
    if (path.exists()) {
        path.mkdirs()
    }
    val file =
